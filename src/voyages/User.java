...
public class User implements IModel {
    ...
    private static String GET_USERS = "SELECT " + columns + " FROM Users";
    private static String GET_BY_ID = "SELECT " + columns + " FROM Users WHERE id = ?";
    private static String GET_BY_EMAIL_PASSWORD = "SELECT * FROM Users WHERE Email = ? AND Password = ?";
    private static String CREATE = "INSERT INTO Users (" + non_id_columns + ") VALUES(?, ?, ?, ?, ?) ";
    ...
    public static User getAuthenticatedUser(HttpServletRequest request) {
        if(request.getSession(true).getAttribute("authUser") != null && request.getSession(true).getAttribute("authUser") instanceof User) {
    ...
    public User(Connexion connexion, String email, String password) {
        this.Email = email;
    ...
    public IModel read(IModel model) throws DAOException {
        try {
            ...
            getByIdStatement.setLong(1, user.id);
            
            ResultSet rset = getByIdStatement.executeQuery();
            if(rset.next()) {
                return extract(rset);
            } else {
                throw new DAOException("User with id " + user.id + " does not exist.");
            }
    ...
    public int create(IModel user) throws DAOException {
        try {
            ...
            createStatement.setString(1, the_user.Email);
            createStatement.setString(2, the_user.Password);
            createStatement.setString(3, the_user.FirstName);
            createStatement.setString(4, the_user.LastName);
            createStatement.setString(5, the_user.Address);
            createStatement.setBoolean(6, the_user.IsAdmin);
    ...
    public int update(IModel user) throws DAOException {
        try {
            ...
            updateStatement.setString(1, the_user.Email);
            updateStatement.setString(2, the_user.Password);
            updateStatement.setString(3, the_user.FirstName);
            updateStatement.setString(4, the_user.LastName);
            updateStatement.setString(5, the_user.Address);
            updateStatement.setBoolean(6, the_user.IsAdmin);            
            updateStatement.setLong(7, the_user.id); 
