...
public class OrderModel implements IModel {
        ...
        private static String GET_ALL = "SELECT " + columns + " FROM Orders";
        private static String GET_BY_ID = "SELECT " + columns + " FROM Orders WHERE OrderId = ?";
        private static String FIND_BY_CUSTOMER = "SELECT " + columns + " FROM Orders WHERE CustomerId = ? ORDER BY OrderId DESC";
        private static String CREATE = "INSERT INTO Orders (" + insert_columns + ") VALUES(?) ";
    ...
    public IModel read(IModel model) throws DAOException {
        try {
            ...
            if(rset.next()) {
                ...
                throw new DAOException("Order with id " + the_model.OrderId + " does not exist.");
            }
    ...
    public int create(IModel model) throws DAOException {
        try {
            ...
            createStatement.setLong(1, the_order.CustomerId);
    ...
    public List<OrderModel> findByCustomer(User user) throws DAOException {
        try {
            ...
            getByCustomerStatement.setLong(1, user.id);
